...
but that also had Microsoft BASIC. Unfortunately, the administration of the BASIC and Kernal was a bit 
different, so it took some time to get "Keeping two programs simultaneously" running on my 
...
The gray parts (zero page, stack, OS data, screen buffer, BASIC, I/O and Kernal) are mostly ignored;
...
The image below shows a simple example (`10 REM A`) in more detail.

![10 REM A](10REM_A.drawio.png)

If a program uses variables (scalars, i.e. not arrays), they are stored from location `VARTAB` 
...
the size and a pointer to the characters, which are on the heap (see later), or part of the program text.

If a program uses array variables, they are store from location `ARYTAB` to location `STREND`.
Every new array pushes up `STREND`. This name is a bit confusing. As mentioned before, string 
...
The table below shows their page addresses.
...
> We will set the pointers tagged "user", then by calling "CLR", the other three pointers will be set.
...
- Each app gets a block of 2k RAM.
...
The partition manager A0 can be loaded from e.g. a disk.
...
its own `MEMSIZ`. Now the three "user" pointers are set.
...
This clears the 2k and initializes `VARTAB`.

Then we program A1. At the end of A1, we set `TXTTAB`, `VARTAB`, and `MEMSIZ`
to the values written down for A0.

In other words, from the table below, the pointers with no marking come for free, 
the pointers with the single < marking come by design (it is the partition we want),
...
Before or after running it, the listing can be saved e.g. to disk. 
BASIC's SAVE will look at `TXTTAB` and `VARTAB` (but not `MEMSIZ`) to determine what to save, 
and `TXTTAB` is even saved as header of the `PRG` file.
...
The first time we need to call `NEW` to set `VARTAB`.

You can now load or enter a program. We start with the latter.

![A1 listing](A1listing.jpg)

Note
- Lines 100 and 110 identify this program as user program A1.
...
Press e.g. space to exit. Save this user program. Rerun it and press `0`. 
After pressing `0` in A1, it activated the partition for A0 again.

Update the `VARTAB` in A0 for A1. Save A0.
...
When loading without the `,1` BASIC will load at `TXTTAB` and automagically fix the linked list of
BASIC line numbers.

The image below shows the hexdump of A1 (version V3).
...
- We see that the next line indeed starts at address 1009, just as the link indicates.
...
- knows that app `C` is current
- that offers an `USR(N)` which
  - saves the current pointers of `C` in the store
  - sets `C` to `N`
  - copies the pointers of the now `C` to the zero page
- the the administration is automated.
