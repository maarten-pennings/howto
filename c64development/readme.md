...
- With a make tool the above steps are automated.

I use Windows for testing
...
- Run `cmd.exe` (or PowerShell) as admin an issue `wsl --install`.
...
a list of bytes prefixed with a load address (C000 in out case).
...
We use `petcat` from VICE to convert this to a standard .prg file.

This second program [`border.bas`](src/border.bas) is a bit more 
complicated, but we feel it is a typical setup: it loads the assembly 
sub-routine, then runs it. 
...
and initialized to 0, so the goto to line 60 will not happen.

Line 20 prints the action that is taken: loading the assembly subroutine.
That actually happens on line 30, setting the load flag `a` to `.
...
C64 BASIC has a bit funny implementation of `LOAD`: after loading the 
...
So after line 30, line 10 will execute, which now does jump to 60.
The second part of our program prints it will call the assembly sub,
then it calls it (49152=$C000), then prints we're back in basic and 
ends the program.
...
To convert [`border.asm`](src/border.asm) to a .prg file, we use `64tass`.
...
To convert [`border.asm`](src/border.bas) to a .prg file, we use `petcat`.
Details are in the [Makefile](makefile).
...
The generates .prg file also starts with a load address, in our case 0801 (red box).
...
then load to the start of BASIC, which happens to be 0801.
...
Maybe later we have other files to load.

The solution is to create a C64 disk image with the tool `c1541`.
It create a new disk, format it and save two .prg files on it.
...
and loads and runs the first .prg file. 
On the screenshot below, we see the `LOAD"*",8,1` and the `RUN` 
caused by double clicking on the .d64 file.

Next we see the `LOAD` message from line 20 of our BASIC program.
This causes a re-run, which jumps to line 60, printing that a
call will be made. The call is made which makes the border white 
as we can see in the screenshot. Finally, 
`80 print "back in basic."` is excuted.

![Executing](images/vice.png)

It is also possible to add this line to the makefile to
start VICE automatically. For me that doesn't work:
